...
Aparecen letras sueltas.`,
  ...
  function swapBillboardSet(index) {
    ...
    if (app.currentScreen === roomScreen && !app.isLoading) {
      window.archiveAudio?.playEffect("glitch");
    }
  ...
  function stopBillboardAnimations() {
    if (billboardTimer) {
    ...
    if (starsTimer) {
      window.clearInterval(starsTimer);
      starsTimer = null;
    }

    if (exitLightTimer) {
      window.clearTimeout(exitLightTimer);
      exitLightTimer = null;
    }
  ...
  function startCatDialogue() {
    ...
    window.archiveAudio?.playEffect("cat");
    hasClickedCat = true;
    clearIdleHintTimer();
    clearBillboardCaptionSequence();
    clearBillboardHintTimers();
